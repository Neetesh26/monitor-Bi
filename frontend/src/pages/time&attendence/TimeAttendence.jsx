...
const generateTimesheetRows = () =>
  Array.from({ length: 6 }, (_, i) => {
    const d = new Date(2026, 2, 6); // Mar 6, 2026
    const checkInH = 9, checkInM = 5 + i * 2;
    const checkOutH = 18, checkOutM = 10 + i * 3;
    const workH = 8, workM = 30 + i * 5;
    return {
      id: i + 1,
      date: "Mar 6, 2026",
      day: "Friday",
      ...
      checkIn: toHHMM(checkInH, checkInM % 60),
      checkOut: toHHMM(checkOutH, checkOutM % 60),
      workTime: toHHMM(workH, workM % 60),
...
const FilterDropdown = ({ onClose, filters, setFilters }) => {
  ...
  return (
    <div style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 220, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.10)", zIndex: 100, padding: 16 }}>
      <p style={{ margin: "0 0 10px", fontSize: 12, fontWeight: 600, color: "#1e293b" }}>Filter by Status</p>
      {statusOptions.map(s => (
        <label key={s} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", cursor: "pointer", fontSize: 13, color: "#374151" }}>
          <span style={{ width: 16, height: 16, border: "1.5px solid #cbd5e1", borderRadius: 4, background: filters.statuses.includes(s) ? "#2563eb" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.1s" }}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button onClick={() => setFilters(f => ({ ...f, statuses: [] }))}
...
const MonthPicker = ({ selected, onSelect, onClose }) => (
  <div style={{ position: "absolute", left: 0, top: "calc(100% + 8px)", width: 180, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.10)", zIndex: 100, overflow: "hidden" }}>
    {MONTHS.map(m => (
      <button key={m} onClick={() => { onSelect(m); onClose(); }}
        style={{ display: "block", width: "100%", textAlign: "left", padding: "9px 14px", border: "none", background: selected === m ? "#eff6ff" : "#fff", fontSize: 13, color: selected === m ? "#2563eb" : "#374151", fontWeight: selected === m ? 600 : 400, cursor: "pointer" }}>
        {m}
      </button>
    ))}
  </div>
...
const TimeAttendence = () => {
  ...
  // Apply status filter to timesheet & attendance
  ...
  return (
    <div style={{
      ...
      {/* ── Header ── */}
      <div style={{ padding: "18px 24px 0", display: "flex", alignItems: "flex-start", gap: 10 }}>
        <button style={{ marginTop: 2, width: 28, height: 28, border: "1px solid #e2e8f0", borderRadius: 8, background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#475569", flexShrink: 0 }}>
          <BackArrow />
        </button>
        <div>
          <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#1e293b" }}>Time & Attendance</h2>
          <p style={{ margin: "3px 0 0", fontSize: 12, color: "#94a3b8" }}>Monitor check-ins, check-outs, and total work hours easily.</p>
        </div>
      </div>

      {/* ── Tabs ── */}
      <div style={{ display: "flex", borderBottom: "1px solid #e2e8f0", marginTop: 16 }}>
        {TABS.map(({ key, label }) => (
      ...
      <div style={{ padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ position: "relative" }}>
          <button onClick={() => { setShowMonthPicker(v => !v); setShowFilter(false); }}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", border: "1px solid #e2e8f0", borderRadius: 8, background: "#fff", fontSize: 12, color: "#374151", cursor: "pointer", fontWeight: 500 }}>
            <CalIcon />
        ...
        {/* Filter button */}
        <div style={{ position: "relative" }}>
          <button onClick={() => { setShowFilter(v => !v); setShowMonthPicker(false); }}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", border: "1px solid #e2e8f0", borderRadius: 8, background: showFilter ? "#eff6ff" : "#fff", fontSize: 12, color: showFilter ? "#2563eb" : "#374151", cursor: "pointer", fontWeight: 500 }}>
            <FilterIcon /> Filter
            {filters.statuses.length > 0 && (
              <span style={{ background: "#2563eb", color: "#fff", fontSize: 10, fontWeight: 700, width: 16, height: 16, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
